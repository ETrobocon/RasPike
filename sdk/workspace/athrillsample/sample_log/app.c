...
static LogDataType logging_data[1024] __attribute__ ((section(".mmap_section")));
static void pug_log(LogDataType *data)
{
	static int count = 0;
	if (count < 1024) {
		logging_data[count] = *data;
		count++;
	}
	return;
}
...
#define black 10
        ...
        static float midpoint = (white - black) / 2 + black;
        {
            ...
            lasterror = error;
        	log_data.step = i;
        	log_data.data1 = error;
        	log_data.data2 = lasterror;
        	log_data.data3 = midpoint;
        	pug_log(&log_data);
        	i++;
        }
